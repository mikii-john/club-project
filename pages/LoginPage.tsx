...
import { BedDouble, Mail, ArrowRight, Loader2 } from 'lucide-react';
...
const LoginPage: React.FC<LoginPageProps> = ({ setUser }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setUser(user);
      navigate('/'); // Redirect to Landing Page after guest login
    } catch (error) {
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col justify-center py-12 sm:px-6 lg:px-8 transition-colors duration-300">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <h2 className="mt-6 text-center text-3xl font-serif font-bold text-slate-900 dark:text-white">
          Grand Horizon Access
        </h2>
        <p className="mt-2 text-center text-sm text-gray-600 dark:text-slate-400">
          Sign in to view your reservation or manage hotel services.
        </p>
      </div>
        ...
        <div className="bg-white dark:bg-slate-900 py-8 px-4 shadow-xl sm:rounded-lg sm:px-10 border border-gray-100 dark:border-slate-800 transition-colors duration-300">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <input
                  ...
                  name="remember-me"
                  type="checkbox"
                  className="h-4 w-4 text-slate-600 focus:ring-slate-500 border-gray-300 dark:border-slate-700 rounded bg-white dark:bg-slate-800"
                />
              ...
              <div className="text-sm">
                <a href="#" className="font-medium text-slate-600 dark:text-amber-400 hover:text-slate-500 dark:hover:text-amber-300">
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-gray-600 dark:text-slate-400">
              ...
              <Link to="/signup" className="font-medium text-slate-600 dark:text-amber-400 hover:text-slate-500 dark:hover:text-amber-300">
                Sign up
              </Link>
            </p>
          </div>
          ...
          <div className="mt-6">
            ...
            <div className="mt-6 grid grid-cols-1 gap-3">
               <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-md text-xs text-slate-500 dark:text-slate-400 text-center">
                 Guests: Use email <strong>guest@grandhorizon.com</strong> <br/>
                 (Any password works)
               </div>
            </div>
          </div>
